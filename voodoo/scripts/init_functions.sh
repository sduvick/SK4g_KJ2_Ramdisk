...
detect_supported_model_and_setup_partitions()
{
	...
	for x in /voodoo/partition_tables/* ; do
        ...
        # Forced model for sidekick only, made this voodoo sidekick exclusive
        model='tmo-sidekick4g'
	...
	if test "$model" != ""; then
		...
		# and also no dbdata
		elif test "$model" = 'tmo-vibrant-galaxys4g' || test "$model" = 'tmo-galaxys4g' || test "$model" = 'tmo-sidekick4g'; then
			dbdata_partition=''
			data_on_emmc=0
